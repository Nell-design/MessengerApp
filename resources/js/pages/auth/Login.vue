...
// Importation des composants réutilisables et utilitaires
...
// Props reçues du parent ou du backend (statut, possibilité de reset password)
...
const submit = () => {
    form.post(route('login'), {
        onFinish: () => form.reset('password'), // On efface le champ mot de passe après soumission
    });
};

// Liste des réseaux sociaux pour affichage des icônes (UI uniquement)
...
<template>
    <!-- Définition du titre de la page pour le SEO et l'accessibilité -->
    <Head title="Log in" />
    <!-- Conteneur principal centré, fond gris clair, padding horizontal pour le mobile -->
    <div class="min-h-screen flex flex-col justify-center items-center bg-gray-50 px-2">
        <!-- Bloc principal centré, largeur max pour le responsive -->
        <div class="w-full max-w-md relative z-10">
            <!-- Carte blanche contenant le formulaire, sans bordure, ombre douce -->
            <form @submit.prevent="submit" class="bg-white rounded-2xl shadow-xl px-6 py-8 mt-20 flex flex-col gap-6 relative">
                <!-- Titre et sous-titre centrés -->
                <div class="text-center mb-4">
                    <h1 class="text-3xl font-extrabold mb-2 tracking-tight text-violet-700">Sign In</h1>
                    <p class="text-base opacity-90 font-medium text-gray-500">Welcome back! Please enter your details.</p>
                </div>
                <!-- Message de statut (ex: succès, erreur) -->
                <div v-if="props.status" class="mb-2 text-center text-sm font-medium text-green-600">
                    {{ props.status }}
                </div>
                ...
                <div class="flex flex-col gap-4">
                    ...
                    <div>
                        ...
                        <Input
                            ...
                            class="rounded-lg border-gray-200 focus:border-violet-500 focus:ring-violet-500"
                        />
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1">
                            ...
                            <!-- Lien mot de passe oublié, affiché si autorisé -->
                            <TextLink v-if="props.canResetPassword" :href="route('password.request')" class="text-xs text-violet-600 font-semibold hover:underline">
                                Forgot Password?
                            </TextLink>
                        </div>
                        <Input
                ...
                <div class="text-center text-sm text-gray-500 mt-4">
                    ...
                    <TextLink :href="route('register')" class="text-violet-600 font-semibold hover:underline">Sign Up</TextLink>
                </div>
            </form>
        </div>
    </div>
